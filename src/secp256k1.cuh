...
// RepresentaÃ§Ã£o de nÃºmeros de 256 bits como 8 x uint32_t (little-endian)
typedef struct {
    uint32_t d[8];
} uint256_t;

// Ponto na curva elÃ­ptica
typedef struct {
    uint256_t x;
    uint256_t y;
    uint256_t z;
...
// 52ab96fe, 2018) - computes a^(p-2) with 253 squarings + 14 multiplications
...
SECP_QUAL void uint256_mod_inv_chain_v2(uint256_t* r, const uint256_t* a, const uint256_t* m) {
    ...
    uint256_mod_mul(&x2, &x2, a, m);              /* x2 = a^3 */

    uint256_mod_sqr(&x3, &x2);
    uint256_mod_mul(&x3, &x3, a, m);

    x6 = x3;
    for (j = 0; j < 3; j++) uint256_mod_sqr(&x6, &x6);
    uint256_mod_mul(&x6, &x6, &x3, m);

    x9 = x6;
    for (j = 0; j < 3; j++) uint256_mod_sqr(&x9, &x9);
    uint256_mod_mul(&x9, &x9, &x3, m);

    x11 = x9;
    for (j = 0; j < 2; j++) uint256_mod_sqr(&x11, &x11);
    uint256_mod_mul(&x11, &x11, &x2, m);

    x22 = x11;
    for (j = 0; j < 11; j++) uint256_mod_sqr(&x22, &x22);
    uint256_mod_mul(&x22, &x22, &x11, m);

    x44 = x22;
    for (j = 0; j < 22; j++) uint256_mod_sqr(&x44, &x44);
    uint256_mod_mul(&x44, &x44, &x22, m);

    x88 = x44;
    for (j = 0; j < 44; j++) uint256_mod_sqr(&x88, &x88);
    uint256_mod_mul(&x88, &x88, &x44, m);

    x176 = x88;
    for (j = 0; j < 88; j++) uint256_mod_sqr(&x176, &x176);
    uint256_mod_mul(&x176, &x176, &x88, m);

    x220 = x176;
    for (j = 0; j < 44; j++) uint256_mod_sqr(&x220, &x220);
    uint256_mod_mul(&x220, &x220, &x44, m);

    x223 = x220;
    for (j = 0; j < 3; j++) uint256_mod_sqr(&x223, &x223);
    uint256_mod_mul(&x223, &x223, &x3, m);

    /* sliding window over the blocks - the final assembly */
    t1 = x223;
    for (j = 0; j < 23; j++) uint256_mod_sqr(&t1, &t1);
    uint256_mod_mul(&t1, &t1, &x22, m);
    for (j = 0; j < 5; j++) uint256_mod_sqr(&t1, &t1);
    uint256_mod_mul(&t1, &t1, a, m);
    for (j = 0; j < 3; j++) uint256_mod_sqr(&t1, &t1);
    uint256_mod_mul(&t1, &t1, &x2, m);
    for (j = 0; j < 2; j++) uint256_mod_sqr(&t1, &t1);   /* THE missing 2 sqrs! */
    uint256_mod_mul(r, a, &t1, m);
}
...
// ============================================================================
// P5: FIXED-BASE WINDOWED G TABLE
// 64 windows x 15 entries: entry (w,d) = d * 2^(4w) * G, affine, d=1..15.
// 60KB, read-only, random per-thread access -> global memory + __ldg
// (constant memory would serialize on divergent addresses).
// Built HOST-side once at startup (build-time injection, GLBP style).
// Replaces the broken d_Gx_table/d_Gy_table (filled with G copies, unused).
// ============================================================================

#define SECP_GWIN_WINDOWS 64
#define SECP_GWIN_ENTRIES (SECP_GWIN_WINDOWS * 15)

#ifdef __CUDACC__
/* uint256_t is a struct - no __ldg overload; plain const deref still emits
   LDG.E on read-only data */
#define SECP_LDG(p) (*(p))
#else
#define SECP_LDG(p) (*(p))
#endif

/* device globals in CUDA; plain definitions under the host test shim */
__device__ uint256_t d_Gwin_x[SECP_GWIN_ENTRIES];
__device__ uint256_t d_Gwin_y[SECP_GWIN_ENTRIES];
...
SECP_QUAL static void secp256k1_gwin_build(uint256_t* xs, uint256_t* ys) {
    ...
    for (int w = 0; w < SECP_GWIN_WINDOWS; w++) {
        point_t acc = B; /* d = 1 */
        for (int d = 1; d <= 15; d++) {
            /* normalize acc to affine */
            uint256_t zinv, z2, z3;
            uint256_mod_inv(&zinv, &acc.z, &SECP256K1_P);
            uint256_mod_mul(&z2, &zinv, &zinv, &SECP256K1_P);
            uint256_mod_mul(&z3, &z2, &zinv, &SECP256K1_P);
            uint256_mod_mul(&xs[w * 15 + (d - 1)], &acc.x, &z2, &SECP256K1_P);
            uint256_mod_mul(&ys[w * 15 + (d - 1)], &acc.y, &z3, &SECP256K1_P);
            if (d < 15) {
                point_t nx;
                point_add(&nx, &acc, &B); /* acc += B */
                acc = nx;
            }
        }
        if (w < SECP_GWIN_WINDOWS - 1) {
            for (int k = 0; k < 4; k++) { /* B *= 16 */
                point_t nb;
                point_double(&nb, &B);
                B = nb;
            }
...
#ifdef __CUDACC__
/* E-fix: the build reads __constant__ symbols (G, P) which are INVALID from
 * host code - so the table is built ON DEVICE by a tiny init kernel writing
 * straight into the device arrays. */
__global__ void secp256k1_gwin_init_kernel(void) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        printf("[GWIN-K] init start: GX.d[0]=%08x GX.d[7]=%08x\n",
               (unsigned)SECP256K1_GX.d[0], (unsigned)SECP256K1_GX.d[7]);
        secp256k1_gwin_build(d_Gwin_x, d_Gwin_y);
        printf("[GWIN-K] done: d_Gwin_x[0].d[0]=%08x d[1]=%08x\n",
               (unsigned)d_Gwin_x[0].d[0], (unsigned)d_Gwin_x[0].d[1]);
    }
}

/* Host: launch the on-device table build. Call once before any kernel. */
static inline void secp256k1_gwin_upload(void) {
    secp256k1_gwin_init_kernel<<<1, 1>>>();
    cudaDeviceSynchronize();
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) printf("[GWIN] init error: %s\n", cudaGetErrorString(err));
}
#endif
...
SECP_QUAL void scalar_mult_G_window(point_t* r, const uint256_t* k) {
    ...
    uint256_clear(&result.y);
    uint256_clear(&result.z);

    #pragma unroll 1
    for (int w = 0; w < SECP_GWIN_WINDOWS; w++) {
        uint32_t nib = (k->d[w >> 3] >> ((w & 7) * 4)) & 0xFu;
        if (nib) {
            point_t T;
            T.x = SECP_LDG(&d_Gwin_x[w * 15 + (nib - 1)]);
            T.y = SECP_LDG(&d_Gwin_y[w * 15 + (nib - 1)]);
            uint256_set_one(&T.z);
            T.infinity = false;
            if (result.infinity) {
                result = T;
