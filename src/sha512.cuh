...
// E19: SHA-512 compression with ROTATING W[16] schedule (registers only).
// Eliminates the W[80] local-memory monster (640B â†’ 32B per chain).
// Same algorithm, same output â€” just the schedule lives in named registers.
__device__ __forceinline__ void sha512_transform_rot16(uint64_t* state, const uint8_t* block) {
    uint64_t W[16];
    ...
    // rounds 0-15: message words directly from the window
    #pragma unroll
    for (int t = 0; t < 16; t++) {
    ...
    // rounds 16-79: rotating schedule in registers
    #pragma unroll
    for (int t = 16; t < 80; t++) {
...
__device__ void sha512(const uint8_t* data, size_t len, uint8_t* hash) {
    ...
    while (len >= 128) {
        sha512_transform_rot16(state, data);
        data += 128;
        len -= 128;
        total_len += 128;
    }
    ...
    if (buffer_len > 112) {
        ...
        sha512_transform_rot16(state, buffer);
        buffer_len = 0;
    }
